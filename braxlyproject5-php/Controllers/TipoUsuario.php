        ...
        foreach($registro as $key => $value){
            if(array_key_exists('Authorization',$headers)&& !empty($headers['Authorization'])){
                ...
                .base64_encode($value['usa_user_secreta'].':'.$value['usa_llave_secreta'])){
                    ...
                    $tipousuario = $model -> where('tipo_estado',1)->findAll();
    ...
    public function show($id){
        ...
        foreach($registro as $key => $value){
            if(array_key_exists('Authorization',$headers)&& !empty($headers['Authorization'])){
                ...
                .base64_encode($value['usa_user_secreta'].':'.$value['usa_llave_secreta'])){
                    $model = new TipoUsuarioModel();
                    $tipousuario = $model ->where('tipo_estado',1)->find($id);
                    //var_dump($curso); die;
                    if(!empty($tipousuario)){
                        $data = array(
                            "Status" => 200,
    ...
    public function create(){
        ...
        foreach($registro as $key => $value){
            if(array_key_exists('Authorization',$headers)&& !empty($headers['Authorization'])){
                ...
                .base64_encode($value['usa_user_secreta'].':'.$value['usa_llave_secreta'])){
                        $datos = array(
                            "tipousa_descripcion" => $request -> getVar("tipousa_descripcion")
                        );
                        if(!empty($datos)){
                            $validation -> setRules([
                                "tipousa_descripcion" => 'required|string|max_length[100]'
                            ]);
                            ...
                            if($validation -> getErrors()){
                                ...
                                $datos = array(
                                    "tipousa_descripcion" => $datos["tipousa_descripcion"]
                                );
                                $model = new TipoUsuarioModel();
