...
use App\Models\TipoPlatoModel;
use App\Models\UsuarioModel;
class TipoPlato extends Controller{
        ...
        foreach($registro as $key => $value){
            if(array_key_exists('Authorization',$headers)&& !empty($headers['Authorization'])){
                ...
                .base64_encode($value['usa_user_secreta'].':'.$value['usa_llave_secreta'])){
                    $model = new TipoPlatoModel();
                    $tipousuario = $model -> where('tipoplato_estado',1)->findAll();
    ...
    public function show($id){
        ...
        foreach($registro as $key => $value){
            if(array_key_exists('Authorization',$headers)&& !empty($headers['Authorization'])){
                ...
                .base64_encode($value['usa_user_secreta'].':'.$value['usa_llave_secreta'])){
                    $model = new TipoPlatoModel();
                    $tipousuario = $model ->where('tipoplato_estado',1)->find($id);
                    //var_dump($curso); die;
                    if(!empty($tipousuario)){
                        $data = array(
                            "Status" => 200,
    ...
    public function create(){
        ...
        foreach($registro as $key => $value){
            if(array_key_exists('Authorization',$headers)&& !empty($headers['Authorization'])){
                ...
                .base64_encode($value['usa_user_secreta'].':'.$value['usa_llave_secreta'])){
                        $datos = array(
                            "tipoplato_nombre" => $request -> getVar("tipoplato_nombre")
                        );
                        if(!empty($datos)){
                            $validation -> setRules([
                                "tipoplato_nombre" => 'required|string|max_length[100]'
                            ]);
                            ...
                            if($validation -> getErrors()){
                                ...
                                $datos = array(
                                    "tipoplato_nombre" => $datos["tipoplato_nombre"]
                                );
                                $model = new TipoPlatoModel();
